 ...
 *
 * 当预估的key数量比较大时用大的初始化量会比rehash有更好的性能
 ...
 *
 * 本实现非线程安全，通常需要借助外部同步方法才能进行同步；
 * 关于结构修改的定义仅为对key的增加或删除，对value的变化不作规定
 ...
 *
 * 可以使用Collections.synchronizedMap装饰HashMap使其成为同步Map
 ...
 *
 * 除了Iterator自身的remove方法，在遍历过程中的任何结构修改都会快速失败
 ...
 * future.
 *
 * 快速失败只尽最大努力保证探测并发修改结构操作，所以不能以其作为一个是否存在并发的因素
 * 但使用其探测一些可能存在的并发修改的bug还是可以的
...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     *
     * key节点作为一个普通Node，其value列表过多时，会转换成TreeNode；判断其Node类型通过instaceof
     * TreeNode用作快速获取其列表值，不过由于key多为普通Node，检查Node类型也许会带来了一些性能减弱
    ...
    static final int hash(Object key) {
        ...
        // 0101 0101 1011 1101 1000 1001 1101 1100 - ^（异或）
